...
export async function mountSettingsCategory({ container, category }) {
    ...
    const renderProcedure = () => {
        ...
        const view = procedureIndicatorView(procedureState, procedure);
        panel.hidden = !view;
        if (view) {
    ...
    const watchProcedure = () => {
        ...
        timer = setInterval(() => {
            ...
            if (procedureState !== previous) renderProcedure();
        }, PROCEDURE_TICK_MS);
    };
    ...
    const onClick = async event => {
        ...
        if (action === 'confirm-procedure') {
            ...
            try {
                ...
                // Stop (or leaving the page) during the wake must not be
                // undone by the start request that was already in flight.
